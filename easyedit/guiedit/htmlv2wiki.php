...
class HTMLV2Wiki
{
	...
	function Div($line) {
        ...
        if(preg_match("/<span\s[^>]*?class=\"embededplugin\".*?>.*?<\/span>/", $line)){
            $line = preg_replace_callback("/<span\s([^>]*?class=\"embededplugin\"[^>]*?)>(.*?)<\/span>/", function($matches){
                if(preg_match("/_pkwkargs=\"([^\"]*?)\"/", $matches[1], $matches2)){$args = $matches2[1];}
                if(preg_match("/_type=\"([^\"]*?)\"/", $matches[1], $matches2)){$type = $matches2[1];}
                if(preg_match("/_plugin=\"([^\"]*?)\"/", $matches[1], $matches2)){$plugin = $matches2[1];}
                if($plugin == "ref"){
                    if(preg_match("/<img\s([^>]*?)\s\/>/", $matches[2], $matches2)){
                        if(preg_match("/src=\"([^\"]*?)\"/", $matches2[1], $matches3)){$src = $matches3[1];}
                        if(preg_match("/style=\"([^\"]*?)\"/", $matches2[1], $matches3)){$style = $matches3[1];}
	...
	function Table($line) {
        ...
        else if (preg_match("/<col\s([^\/]*?)\/>(.*)/", $line, $matches) && $colgroup_matching == true) {
            $line = $matches[2];
